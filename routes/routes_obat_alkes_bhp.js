...
router.get('/', middleware.asyncMiddleware(async (req, res, next) => {
    const allobat = await Obat.find({}).populate("id_kategori_barang").populate("id_satuan_barang");
...
router.get("/:id/edit", middleware.asyncMiddleware(async (req, res, next) => {
    const cari_obat = await Obat.findById(req.params.id).populate("id_kategori_barang").populate("id_satuan_barang");
    const data_kategori_barang = await Kategori.find({});
    const data_satuan_barang = await Satuan.find({});
